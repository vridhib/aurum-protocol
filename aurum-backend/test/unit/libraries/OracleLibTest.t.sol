...
contract OracleLibExposer {
    using OracleLib for AggregatorV3Interface;

    function staleCheckLatestRoundData(address priceFeed)
        external
        view
        returns (uint80, int256, uint256, uint256, uint80)
    {
        return AggregatorV3Interface(priceFeed).staleCheckLatestRoundData();
    }
}
...
contract OracleLibTest is StdCheats, Test {
    ...
    function testPriceRevertsOnStaleCheck() public {
        ...
        exposer.staleCheckLatestRoundData(address(aggregator));
    }
    ...
    function testStaleCheckWorksWhenPriceIsFresh() public view {
        (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound) = exposer.staleCheckLatestRoundData(address(aggregator));
    ...
    function testStaleCheckPassesAtExactly30Days() public {
        // Warp time forward by exactly 30 days
    ...
    function testStaleCheckFailsAt30DaysAnd1Second() public {
